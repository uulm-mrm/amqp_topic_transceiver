...
AMQPTopicTransmitter::AMQPTopicTransmitter(ros::NodeHandle node_handle, ros::NodeHandle private_node_handle)
  : private_nh_(private_node_handle), dyn_param_server_(private_nh_), nh_(node_handle), metadata_sent(false)
{
  ...
  sub_ = node_handle.subscribe(
      "/msg", 10, &AMQPTopicTransmitter::processMessage, this, ros::TransportHints().tcpNoDelay());
}
...
void AMQPTopicTransmitter::processMessage(const ros::MessageEvent<topic_tools::ShapeShifter>& msg_event)
{
