...
AMQPTopicReceiver::AMQPTopicReceiver(ros::NodeHandle node_handle, ros::NodeHandle private_node_handle)
  : private_nh_(private_node_handle), dyn_param_server_(private_nh_), nh_(node_handle), publisher_created(false)
{
...
bool AMQPTopicReceiver::run()
{
  ...
  bool latch = false;
  {
    ...
    amqp_basic_consume(conn, 1, queuename, consumer_tag, 0, 1, 0, amqp_empty_table);
